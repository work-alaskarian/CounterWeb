...
const API_URL = import.meta.env?.VITE_API_URL || 'http://10.10.1.205:8080';
const GRAPHQL_ENDPOINT = `${API_URL}/graphql`;
const WEBSOCKET_URL = import.meta.env?.VITE_WEBSOCKET_URL || 'ws://10.10.1.205:8080/ws/analytics';
...
console.log('🔧 Analytics API Configuration:', {
  ...
  WEBSOCKET_URL,
  fromEnv: {
    API_URL: import.meta.env?.VITE_API_URL ? '✅' : '❌ (using fallback)',
    WEBSOCKET_URL: import.meta.env?.VITE_WEBSOCKET_URL ? '✅' : '❌ (using fallback)'
  }
});
...
class AnalyticsAPI {
  ...
  connectWebSocket() {
    ...
    try {
      ...
      this.websocket.onopen = () => {
        ...
        // Subscribe to live count pattern for real-time updates
        this.subscribeToLiveCountPattern(this.currentTimeframe);
  ...
  /**
   * Subscribe to live count pattern for real-time updates (NEW)
   */
  subscribeToLiveCountPattern(timeframe = 'HOURLY', locationId = 'all', interval = 3) {
    this.sendWebSocketMessage({
      action: 'subscribe_pattern',
      pattern: 'live_count',
      locationId: locationId,
      interval: interval, // seconds between updates
      timeframe: timeframe
    });
    console.log(`🔔 Subscribed to live_count pattern: ${locationId} (${timeframe}, ${interval}s interval)`);
  }

  /**
   * Subscribe to chart data pattern for chart updates (NEW)
   */
  subscribeToChartDataPattern(timeframe = 'HOURLY', locationId = 'all', interval = 10, limit = 20) {
    this.sendWebSocketMessage({
      action: 'subscribe_pattern',
      pattern: 'chart_data',
      locationId: locationId,
    ...
    console.log(`🔔 Subscribed to chart_data pattern: ${locationId} (${timeframe}, ${interval}s interval, ${limit} points)`);
  }

  /**
   * Subscribe to specific location with timeframe (UPDATED for patterns)
   */
  subscribeToLocation(locationId, timeframe = 'HOURLY', interval = 3) {
    this.subscribeToLiveCountPattern(timeframe, locationId, interval);
  }

  /**
   * Update timeframe for existing subscriptions (UPDATED for patterns)
   */
  updateTimeframe(timeframe) {
    this.currentTimeframe = timeframe;

    // Re-subscribe to live count pattern with new timeframe
    this.subscribeToLiveCountPattern(timeframe);

    console.log(`📅 Updated live count subscription timeframe to: ${timeframe}`);
  }
